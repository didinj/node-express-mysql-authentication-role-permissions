...
exports.postgresOptions = {
    ...
    countTriggerQuery: (tableName, schemaName) => {
        return `SELECT COUNT(0) AS trigger_count
             ...
             WHERE t.event_object_table = ${dialect_options_1.addTicks(tableName)}
                  ${dialect_options_1.makeCondition("t.event_object_schema", schemaName)}`;
    },
    /**
     ...
     */
    isSerialKey: (record) => {
        const isSequence = (val) => !!val && ((lodash_1.default.startsWith(val, 'nextval') && lodash_1.default.includes(val, '_seq') && lodash_1.default.includes(val, '::regclass')) || (val === 'ALWAYS' || val === 'BY DEFAULT'));
        return (lodash_1.default.isObject(record) && (isSequence(record.extra) || isSequence(record.defaultValue) || isSequence(record.generation)));
    },
    /**
